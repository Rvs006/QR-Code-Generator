...
├── components/
│   ├── AppIcon.tsx            # Animated QR scanner icon (light/dark PNG variants)
...
│   ├── pdf-exporter.ts        # PDF label sheet export with configurable paper size
...
- **Duplicate asset tag detection**: Enhanced modal with summary stats (total rows / unique tags / would be removed), severity color-coding (mild=yellow, moderate=orange, heavy=red), searchable tag list, hover preview showing removal count. Actions: Keep All / Keep First Only / Keep Last Only / Cancel.
...
- Dual export: ZIP (folder structure) + PDF (configurable paper size with crop marks)
...
- **Guided Tour**: Pulsing blue highlight ring walks users through app features. Auto-triggers for first-time users (per phase: landing/loaded/results). "Take a Tour" button on landing page for manual re-trigger. Stored in `ec-tour-landing`, `ec-tour-loaded`, `ec-tour-results` localStorage keys. Component: `GuidedTour.tsx`.
...
- AppIcon component: uses `@assets/image_1772575316478.png` (light) and `@assets/Screenshot_2026-03-03_221155-removebg-preview_1772575950459.png` (dark)
